...
const AdminMessaging = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState('tutor-messages');
  ...
  const loadMessages = async () => {
    try {
      setLoading(true);
      if (activeTab === 'tutor-messages') {
        const response = await getTutorMessages({ page: 1, limit: 50 });
        if (response.success) {
          setTutorMessages(response.data);
        }
  ...
  const handleReplyToTutor = async () => {
    try {
      await replyToAdmin({
      ...
      setReplyData({ content: '', messageType: 'admin_reply' });
      loadMessages();
    } catch (error) {
  ...
  const handleSendToTutor = async () => {
    try {
      await sendMessageToTutor(composeData.tutorId, composeData);
  ...
  const filteredTutorMessages = tutorMessages.filter(message => {
  ...
  const sortedTutorMessages = [...filteredTutorMessages].sort((a, b) => {
    switch (sortBy) {
      case 'date':
  ...
  const tabs = [
    { 
      id: 'tutor-messages', 
      label: 'Messages from Tutors', 
      count: tutorMessages.length,
      icon: <FaUserTie className="w-4 h-4" />
    },
    { 
      id: 'sent-messages', 
      label: 'Messages Sent to Tutors', 
      count: sentMessages.length,
      icon: <FaEnvelopeOpen className="w-4 h-4" />
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Admin Messaging Center</h1>
          <p className="text-slate-600">Communicate with tutors and manage platform communications</p>
        </div>
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-3">
            ...
            >
              ...
              <span>Send Message to Tutor</span>
            </button>
          </div>
          <div className="text-sm text-slate-500">
            {activeTab === 'tutor-messages' 
              ? `${sortedTutorMessages.length} messages from tutors`
              : `${sortedSentMessages.length} messages sent to tutors`
            }
          </div>
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            sortedTutorMessages.length > 0 ? (
              sortedTutorMessages.map((message) => (
                <motion.div
                  key={message._id}
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-slate-900 mb-2">No messages from tutors</h3>
                <p className="text-slate-600">You haven't received any messages from tutors yet.</p>
              </div>
            )
          ) : (
            sortedSentMessages.length > 0 ? (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          ...
                          <div>
                            <h3 className="text-lg font-semibold text-slate-900">
                              To: {message.receiver?.name || 'Tutor'}
                            </h3>
                            <p className="text-sm text-slate-500">
                              {formatDate(message.createdAt)}
                            </p>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-slate-600">You haven't sent any messages to tutors yet.</p>
                <button
                  onClick={() => setShowComposeModal(true)}
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              <div className="p-6 border-b border-slate-200">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-slate-900">Send Message to Tutor</h3>
                  <button
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={composeData.tutorId}
                    onChange={(e) => setComposeData({ ...composeData, tutorId: e.target.value })}
        ...
        {showReplyModal && selectedMessage && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              <div className="p-6 border-b border-slate-200">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-slate-900">Reply to Tutor</h3>
                  <button
